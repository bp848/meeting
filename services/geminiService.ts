 ...
 */
export const analyzeTranscript = async (transcript: string, userNames: string[] | undefined): Promise<AnalysisResult> => {
  const ai = getAiClient();
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: transcript,
      config: {
        systemInstruction: buildAnalysisSystemInstruction(userNames), // Use dynamic system instruction
        thinkingConfig: {
